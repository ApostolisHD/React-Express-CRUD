...
const getEmployee = async(request, response) => {
  try {
    const results = await pool.query("SELECT id, last_name, first_name, is_active,to_char(date_of_birth, 'DD/MM/YYYY')" +
        " AS date_of_birth FROM employee ORDER BY id ASC ");
    response
      .status(200)
      .json(results.rows);
  } catch (error) {
...
const getEmployeeById = async(request, response) => {
  ...
  try {
    const results = await pool.query("SELECT id, last_name, first_name, is_active,date_of_birth FROM employee WHERE id" +
        "=$1",
    [id]);
    response
      .status(200)
...
const updateEmployee = async(request, response) => {
  ...
  const {last_name, first_name, date_of_birth, is_active} = request.body;
  try {
    const results = await pool.query("UPDATE employee SET last_name= $1, first_name= $2, is_active=$3, date_of_birth=$" +
        "4 WHERE id = $5",
